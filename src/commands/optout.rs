...
pub async fn optout(ctx: Context<'_>) -> Result<(), Error> {
    ...
    deny(
        ctx,
        "**Auto-embedding** has been disabled for you. I'll leave your messages alone, but slash \
         commands you run yourself still work. Use `/optin` to reverse this.",
    )
    .await
}
...
pub async fn optin(ctx: Context<'_>) -> Result<(), Error> {
    ...
    deny(
        ctx,
        "**Auto-embedding** has been enabled for you. I'll process your messages again.",
    )
    .await
}
