...
pub async fn get_server_settings(
    ...
    let result = sqlx::query(
        r#"
        SELECT guild_id, git_diffs_enabled, git_compares_enabled, git_links_enabled, twitter_enabled, tiktok_enabled, instagram_enabled, reddit_enabled
    ...
    if let Some(row) = result {
        Ok(ServerSettings {
            ...
            reddit_enabled: row.try_get("reddit_enabled")?,
        })
    } else {
        Ok(ServerSettings {
...
pub async fn update_server_setting(
    ...
    let query = match setting {
        ...
        "reddit" => sqlx::query(
            r#"
                INSERT INTO server_settings (guild_id, reddit_enabled)
                VALUES ($1, $2)
                ON CONFLICT (guild_id)
                DO UPDATE SET reddit_enabled = $2, updated_at = NOW()
                "#,
        )
        .bind(guild_id)
        .bind(enabled),
